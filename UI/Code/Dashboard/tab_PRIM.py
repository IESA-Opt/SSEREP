...
from Code.Dashboard.tab_paper_plots import apply_default_data_filter
...
def get_unit_for_column(column_name, parameter_lookup=None, outcome_options=None, df_raw=None):
    """Get appropriate units for a column, reading from data when possible.
    
    Args:
        column_name: Raw column name from the dataframe
        parameter_lookup: Parameter lookup table with Unit column (for parameters)
        outcome_options: List of outcome column names
        df_raw: Raw data with Unit column (for outcomes)
        
    Returns:
        Unit string like "[M€]", "[GW]", etc., or empty string if no unit
    """
    ...
    if parameter_lookup is not None and not parameter_lookup.empty:
        try:
            ...
            # If there's any error accessing the parameter lookup, continue to next method
            pass
    
    # Second, try to get unit from raw data for outcomes
    if df_raw is not None and column_name in outcome_options:
        try:
            if 'Outcome' in df_raw.columns and 'Unit' in df_raw.columns:
                # Get rows for this outcome
                outcome_rows = df_raw[df_raw['Outcome'] == column_name]
                if not outcome_rows.empty:
                    # Get the first non-null unit value
            ...
            # If there's any error accessing raw data, continue to fallback
            pass
    
    # Fallback to pattern matching if unit not found in data
    # Determine if this is an outcome or parameter for different unit logic
    is_outcome = outcome_options is not None and column_name in outcome_options
    
    col_lower = column_name.lower()
    
    if is_outcome:
        # Outcome unit logic (fallback patterns)
        if "totalcosts" in col_lower or 'cost' in col_lower:
            return '[M€]'
        elif 'capacity' in col_lower or 'cap' in col_lower:
            return '[GW]'
        elif 'production' in col_lower or 'import' in col_lower or 'export' in col_lower:
            return '[PJ]'
        elif 'share' in col_lower or 'ratio' in col_lower:
            return '[%]'
        elif 'emission' in col_lower or 'co2' in col_lower:
            return '[Mt CO2]'
        elif 'energy' in col_lower:
            return '[PJ]'
        elif 'flexibility' in col_lower:
            return '[GW]'
        ...
        # Parameter unit logic (fallback if not found in lookup)
        if 'price' in col_lower or 'cost' in col_lower:
            return '[€/unit]'
        elif 'capacity' in col_lower or 'cap' in col_lower:
            return '[GW]'
        elif 'ratio' in col_lower or 'share' in col_lower or 'fraction' in col_lower:
            return '[%]'
        elif 'volume' in col_lower or 'storage' in col_lower:
            return '[PJ]'
        elif 'demand' in col_lower or 'consumption' in col_lower:
            return '[PJ/year]'
        elif 'efficiency' in col_lower:
            return '[%]'
...
def _run_prim(x_clean: pd.DataFrame, y_clean: np.ndarray, mass_min: float, peel_alpha: float, paste_alpha: float):
    ...
    This is a small wrapper that protects against errors and returns empty results on failure.
    """
    # Local imports to avoid relying on module-level import order
    try:
        import pandas as pd
    except Exception:
        pd = None
